    ...
    <div class="space-y-6">
      ...
      <ui-panel heading="What this is">
        <p class="text-sm text-slate-700">
          A navigable test harness for the Angular + Claude Agent SDK Python section of the
          CopilotKit docs. Every guide listed in the sidebar is a route, and each route runs the
          thing its doc page teaches rather than restating it. Docs were last synced on
          {{ docSyncDate }}.
        </p>
...
Copilot Runtime  ·  localhost:8200        &#8592; Node, frontend/server.ts
  ...
  v
Claude Agent SDK Python agent · localhost:8000   &#8592; Python / FastAPI
...
Model  (claude-sonnet-4-6)</code></pre>
        ...
        <p class="mt-3 text-sm text-slate-700">
          ...
          key only ever reaches the Claude Agent SDK Python process.
        </p>
      </ui-panel>

      <ui-callout title="Both backends must be running">
        The chat will not stream if either process is down. Start the Claude Agent SDK Python agent
        with
        <code>uv run uvicorn main:app --port 8000</code> from <code>backend/</code>, and the runtime
        with <code>npm run runtime</code> from <code>frontend/</code>.
      </ui-callout>
      ...
      <ui-panel heading="The runtime binding">
        <p class="mb-3 text-sm text-slate-700">
          This is the one file that ties CopilotKit to Claude Agent SDK Python. It is read off disk
