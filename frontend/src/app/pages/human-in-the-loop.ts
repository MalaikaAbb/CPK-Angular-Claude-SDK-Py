  ...
  template: `
    ...
    <div class="space-y-6">
      ...
      <ui-callout title="No Python declaration is needed for this tool">
        ...
        <code>backend/main.py</code>. The inverse does bite: a tool declared on the agent with no
      ...
      <ui-panel heading="The interrupt controller">
        <p class="mb-3 text-sm text-slate-700">
          ...
          emits an AG-UI interrupt (or the legacy
          <code>on_interrupt</code> custom event). The Claude Agent SDK Python agent in this repo
          does not emit one, so this half stays idle. The controller clears stale decisions when the
          thread changes, and <code>resolve</code>/<code>cancel</code> share one in-flight resume
