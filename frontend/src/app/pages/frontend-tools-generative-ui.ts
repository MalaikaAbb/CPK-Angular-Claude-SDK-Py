    ...
    <div class="space-y-6">
      <ui-try-it>
        ...
        <p class="mt-2 text-[var(--ink-soft)]">
          ...
          <em>Show me incident INC-4711 at sev1</em> — an <code>IncidentCardComponent</code> renders
          the id in bold with the severity beside it, and the agent writes no tool result of its
          own. <strong>Fail:</strong> the incident comes back described in prose, which means
          <code>show_incident</code> was never called.
        </p>
        <p class="mt-2 text-[var(--ink-soft)]">
      ...
      <ui-panel heading="registerRenderToolCall — server-side tool, browser renders">
        <p class="text-sm text-[var(--ink-soft)]">
          The Claude Agent SDK Python agent owns the <code>getWeather</code> tool; the browser only
          renders its call. The registration is in <code>tools-chat.component.ts</code>, shown in
          full below alongside the browser-executed tool.
        </p>
      </ui-panel>
      ...
      <ui-callout tone="warn" title="The renderer name must equal the tool name">
        ...
        exact string. The Claude Agent SDK Python agent in <code>backend/main.py</code> has to
      ...
      <ui-panel heading="registerComponent — display only, nothing on the agent side">
        <p class="mb-3 text-sm text-[var(--ink-soft)]">
          The simplest generative UI there is, and the only kind that needs nothing on the agent
          side. <code>show_incident</code> has no handler and no matching tool in
          <code>backend/main.py</code>: the frontend declares it, CopilotKit forwards it to the
          agent over AG-UI, and the agent calls it purely to put the component on screen. Core
          inserts an empty tool result, so the turn completes without an invented result being
        ...
        <p class="mt-4 text-sm text-[var(--ink-soft)]">
          Contrast <code>registerRenderToolCall</code> above, which draws a tool the agent already
          owns and so requires that tool to exist in the Claude Agent SDK Python agent. The
          registration is in <code>tools-chat.component.ts</code>, shown in full below.
        </p>
      </ui-panel>

      <ui-callout tone="warn" title="A well-formed component is not a correct one">
        The model fills these props from what it knows. A card rendered over records this
        application does not hold looks the same in the browser, in a screenshot, and in a video as
        a correct one. Share the page's data with the agent through
        <code>CopilotKitAgentContext</code> — the Shared state route does exactly that — then read
        the rendered fields back against the records you hold.
      </ui-callout>
